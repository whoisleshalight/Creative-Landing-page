    ...
    window.addEventListener("load", (function() {
        setTimeout((function() {
            ...
            scroll.on("scroll", (obj => {
                if (obj.scroll.y >= 10) document.querySelector(".header").classList.add("goo"); else document.querySelector(".header").classList.remove("goo");
            }));
            document.documentElement.classList.add("loaded");
        }), 1e3);
        setTimeout((() => {
            document.documentElement.classList.add("view_main");
        }), 1300);
        const videoWrapper = document.querySelector(".promo-services");
        if (videoWrapper) {
            const videoPlayRotateBtn = document.querySelector(".content-controls__btn-rotate-play");
            const videoPlayBtn = document.querySelector(".controls__play");
            let videoItem = document.querySelector("video");
            const videoStopBtn = document.querySelector(".controls__stop");
            videoStopBtn.addEventListener("click", stopVideo);
            videoPlayRotateBtn.addEventListener("click", playVideo);
            videoPlayBtn.addEventListener("click", playVideo);
            function playVideo() {
                videoWrapper.classList.add("move");
                setTimeout((() => {
                    videoItem.play();
                }), 600);
            }
            function stopVideo() {
                videoItem.pause();
                videoWrapper.classList.remove("move");
            }
        }
    }));
    window["FLS"] = true;
